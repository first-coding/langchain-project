...
class SourceService(object):
    ...
    def init_source_vector(self): #指定特定目录加载所有txt，归档文件对象，通过向量表示，存储在FAISS向量存储中并保存到指定路径
        """
        初始化本地知识库向量
        :return:
        """
    ...
    def add_document(self, document_path): #添加新文档到知识库中
        loader = UnstructuredFileLoader(document_path, mode="elements")
        doc = loader.load()
        self.vector_store.add_documents(doc)
        self.vector_store.save_local(self.vector_store_path)

    def load_vector_store(self, path):#加载本地向量存储
        if path is None:
    ...
    def search_web(self, query):#网路搜索功能
        #网络请求代理设置
        # SESSION.proxies = {
